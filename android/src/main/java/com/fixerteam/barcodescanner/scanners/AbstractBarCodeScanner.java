...
public abstract class AbstractBarCodeScanner implements BarCodeScanner {

  protected Context mContext;
  protected List<Integer> mBarCodeTypes;

  AbstractBarCodeScanner(Context context) {
    mContext = context;
  }
  ...
  List<Integer> parseBarCodeTypesFromSettings(BarCodeScannerSettings settings) {
    ...
    if (!(newBarCodeTypesObject instanceof List)) {
      return null;
    }
    List<Integer> result = new ArrayList<>();
    List<Object> newBarCodeTypesObjectsList = (List<Object>) newBarCodeTypesObject;
